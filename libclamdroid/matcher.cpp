...
//static inline
int MatcherController::matcherRun(const struct cli_matcher *root,
 ...
 */
int MatcherController::cliCaloff(const char *offstr, const struct cli_target_info *info, unsigned int target, uint32_t *offdata, uint32_t *offset_min, uint32_t *offset_max)
{
    char offcpy[65];
    unsigned int n, val;
    char *pt;
...
void MatchController::cliTargetInfo(struct cli_target_info *info, unsigned int target, fmap_t *map)
{
...
int MatcherController::cliCheckFp(unsigned char *digest,
        size_t size,
        cli_ctx *ctx)
{
    char md5[33];
    ...
    if(cli_hm_scan(digest, size,
            &virname,
            ctx->engine->hm_fp,
            CLI_HASH_MD5) == CL_VIRUS) {
    ...
    if(have_sha1 || have_sha256) {
        if((ptr = fmap_need_off_once(map, 0, size))) {
            if(have_sha1) {
                ...
                if(cli_hm_scan(&shash1[SHA1_HASH_SIZE],
                        size, &virname,
                        ctx->engine->hm_fp,
                ...
                if(cli_hm_scan_wild(&shash1[SHA1_HASH_SIZE],
                        &virname,
                        ctx->engine->hm_fp,
                        CLI_HASH_SHA1) == CL_VIRUS) {
                    cli_dbgmsg("cli_checkfp(sha1): Found false positive detection (fp sig: %s)\n", virname);
                ...
                if(do_dsig_check && cli_hm_scan(&shash1[SHA1_HASH_SIZE],
                        1,
                        &virname,
                        ctx->engine->hm_fp,
                        CLI_HASH_SHA1) == CL_VIRUS) {
            ...
            if(have_sha256) {
                ...
                if(cli_hm_scan(&shash256[SHA256_HASH_SIZE],
                        size,
                        &virname,
                        ctx->engine->hm_fp,
                        CLI_HASH_SHA256) == CL_VIRUS) {
                    cli_dbgmsg("cli_checkfp(sha256): Found false positive detection (fp sig: %s)\n", virname);
                    return CL_CLEAN;
                }

                if(cli_hm_scan_wild(&shash256[SHA256_HASH_SIZE],
                        &virname,
                        ctx->engine->hm_fp,
                        CLI_HASH_SHA256) == CL_VIRUS) {
                    cli_dbgmsg("cli_checkfp(sha256): Found false positive detection (fp sig: %s)\n", virname);
    ...
    if(do_dsig_check || ctx->engine->cb_stats_add_sample) {
        ...
        if (!(ctx->engine->engine_options & ENGINE_OPTIONS_DISABLE_PE_STATS)
                && !(ctx->engine->dconf->stats
                        & (DCONF_STATS_DISABLED | DCONF_STATS_PE_SECTION_DISABLED)))
    ...
    if (ctx->engine->cb_hash)
        ctx->engine->cb_hash(fmap_fd(*ctx->fmap),
                size,
                (const unsigned char *)md5, cli_get_last_virus(ctx), ctx->cb_ctx);

    if (ctx->engine->cb_stats_add_sample)
        ctx->engine->cb_stats_add_sample(cli_get_last_virus(ctx),
                digest,
                size,
                &sections,
                ctx->engine->stats_data);
...
int MatcherController::cliScanBuff(const unsigned char *buffer,
        uint32_t length,
        uint32_t offset,
        cli_ctx *ctx,
        cli_file_t ftype,
        struct cli_ac_data **acdata)
{
    ...
    if(troot) {
        ...
        if(!acdata && (ret = cli_ac_initdata(&mdata,
                troot->ac_partsigs,
                troot->ac_lsigs,
                troot->ac_reloff_num,
        ...
        ret = matcher_run(troot,
                buffer,
                length,
                &virname,
                acdata ? (acdata[0]): (&mdata),
                offset,
                NULL,
                ftype,
                NULL,
    ...
    ret = matcher_run(groot,
            buffer,
            length,
            &virname,
            acdata ? (acdata[1]): (&mdata),
            offset,
            NULL,
            ftype,
            NULL,
...
//static
int MatcherController::matchicon(cli_ctx *ctx,
...
int32_t MatcherController::cliBcapiMatchicon(struct cli_bc_ctx *ctx , const uint8_t *grp1, int32_t grp1len,
...
int MatcherController::cliScandesc(int desc,
        cli_ctx *ctx,
        cli_file_t ftype,
        uint8_t ftonly,
        struct cli_matched_type **ftoffset,
...
//static
int MatcherController::lsigEval(cli_ctx *ctx,
    ...
    if (cli_ac_chklsig(exp, exp_end, acdata->lsigcnt[lsid], &evalcnt, &evalids, 0) == 1) {
        ...
        if(ac_lsig->tdb.icongrp1 || ac_lsig->tdb.icongrp2) {
            ...
            if(matchicon(ctx,
                    ...
                    ac_lsig->tdb.icongrp2) == CL_VIRUS) {
                if(!ac_lsig->bc_idx) {
                    cli_append_virus(ctx, ac_lsig->virname);
                    return CL_VIRUS;
                } else if(cli_bytecode_runlsig(ctx, target_info,
                        &ctx->engine->bcs,
                        ac_lsig->bc_idx,
                        acdata->lsigcnt[lsid],
                        acdata->lsigsuboff_first[lsid],
                        map) == CL_VIRUS) {
                    return CL_VIRUS;
                }
            }
...
#ifdef HAVE_YARA
static int yara_eval(cli_ctx *ctx,
        struct cli_matcher *root,
        struct cli_ac_data *acdata,
        struct cli_target_info *target_info,
        const char *hash,
...
int MatcherController::cliExpEval(cli_ctx *ctx,
        struct cli_matcher *root,
        struct cli_ac_data *acdata,
        struct cli_target_info *target_info, const char *hash)
{
...
#ifdef HAVE_YARA
        else if (root->ac_lsigtable[i]->type == CLI_YARA_NORMAL
                || root->ac_lsigtable[i]->type == CLI_YARA_OFFSET)
...
int MatcherController::cliFmapScanDesc(cli_ctx *ctx,
        cli_file_t ftype,
        uint8_t ftonly,
        struct cli_matched_type **ftoffset,
        unsigned int acmode,
        struct cli_ac_result **acres,
        unsigned char *refhash)
{
    ...
    if(!ftonly) {
        if((ret = cli_ac_initdata(&gdata,
                groot->ac_partsigs,
                groot->ac_lsigs,
                groot->ac_reloff_num,
                CLI_DEFAULT_AC_TRACKLEN)) || (ret = cli_ac_caloff(groot, &gdata, &info))) {
    ...
    if(troot) {
        if((ret = cli_ac_initdata(&tdata,
                troot->ac_partsigs,
                troot->ac_lsigs,
                troot->ac_reloff_num, CLI_DEFAULT_AC_TRACKLEN)) ||
...
int MatcherController::cliMatchMeta(cli_ctx *ctx,
        const char *fname,
        size_t fsizec,
        size_t fsizer,
        int encrypted,
        ...
        void *res2)
{
    ...
    if (ctx->engine && ctx->engine->cb_meta)
        if (ctx->engine->cb_meta(cli_ftname(ctx->container_type),
                fsizec,
                fname,
                fsizer,
                encrypted,
